...
area_angel = {'A-1' : {'x': 0, 'y': 0}, 
            ...
            'A-1-11' : {'x': 25, 'y': 1.5},
            'A-11-1' : {'x': 25, 'y': 1.5},
            # 2 sensors overlapped in one point for side A
            'A-1-3' : {'x': 2.5, 'y': 0},
            'A-3-1' : {'x': 2.5, 'y': 0},
            'A-2-4' : {'x': 2.5, 'y': 7},  
            'A-4-2' : {'x': 2.5, 'y': 7},
...
def locate_and_predict(cur_side, cur_sensor, cur_time, next_time, filename, counter):
    ...
    # the animal was residing in an overlapped area
        ...
        area_number = cur_side + '-' + cur_sensor + '-' + pre_sensor
        animal_x = area_angel[area_number]['x']
        animal_y = area_angel[area_number]['y']
...
c = 0      
# for filename in os.listdir("/Users/jinpeng/Workspace/Research/Smart Agriculture/smart_agriculture_edge_server/simulate_prediction/layout3/"):
for filename in os.listdir("/predict/layout3/"):
    if filename.endswith('.csv'):
        f = open("/predict/layout3/" + filename, "r")
        ff = csv.reader(f)
        row = next(ff)
        c += 1
        pre_time_1 = -1000
